...
/** 相机方向在起始局部系下的单位向量。Controller 自己算，不依赖 GizmoFrame 内部量。 */
export function computeToCameraLocal(
...
/**
 * 局部系求交面法线。面必须包含整个自由子空间。
 *
 *   translate/scale + axis（余维 2，自由轴 a）：n_L = normalize(v − (v·a)a)
 *     含 a 的平面有一族，取最正对相机的那张，保证拖拽时分量不退化
 *   rotate + axis：n_L = a
 *     旋转平面的法线就是旋转轴本身
 *   plane（余维 1，法线 c）：n_L = c
 *   view / uniform（余维 0）：n_L = v，退化到视平面
 */
export function resolveDragPlaneNormalLocal(
  mode: ControlMode,
  ...
  switch (constraint.kind) {
    case 'axis': {
      ...
      const axis = constraint.axisLocal
...
export interface BaseDetailSeed extends BaseInteractionDetail {
  /** 起始局部系下的相机方向，子类构造自身 detail 时复用。 */
...
/**
 * 冻结 TRS 与视线，建立求交面并求起始交点。
 * 法线退化（|dot(n_L, v)| < degenerateThreshold）或无交点时返回 null。
 */
export function createBaseDetail(
  ...
  const planeNormalWorld = Matrix3.multiplyByVector(
    rotation,
    planeNormalLocal,
    new Cartesian3(),
  )
...
/**
 * 当前帧 gizmo 的屏幕均匀尺度。gizmoMatrix = [R · s | T]，三列等长，
 * 世界图元乘上它就能保持屏幕恒定大小。
 */
